...
def correlogram(t, title=None, valmin=-1, valmax=1):
    ...
    # sb.heatmap(data=np.round(t, 2), vmin=valmin, vmax=valmax, cmap='PRGn', annot=True) # np.round(t,2)
    sb.heatmap(data=np.round(t, 2), vmin=valmin, vmax=valmax, cmap='BrBG_r', annot=True)  # np.round(t,2)
    plt.title(title)
    plt.show()
...
def corrCircle(R, k1, k2, title="The Correlation Circles"):
    ...
    #     plt.text(np.round(R[i, k1], 1), np.round(R[i, k2], 1), np.round(R[i], 1))
    #     # plt.text(np.round(R[i, k1], 2), np.round(R[i, k2], 2))
    plt.show()